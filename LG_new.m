function dydt = LG(t, y, p)
    ...
    v_sB = p(51);
    v_sC = p(52);
    % ---------- 新增：12h:12h Light/Dark 方波输入 ----------
% Per 基因转录速率 v_sP：光照 1.8，黑暗 1.5
if mod(t, 24) < 12    % t 用小时；0≤t_mod<12 为“光”
    current_v_sP = 1.8;   % Light phase
else
    current_v_sP = 1.5;   % Dark phase
end
% --------------------------------------------------------

% =======  把 Per mRNA 方程中的 v_sP 换成 current_v_sP  =======
% 原来：
% dydt(1) = v_sP * (B_N^n)/(K_AP^n + B_N^n) - ...
% 改成：
dydt(1) = current_v_sP * (B_N^n)/(K_AP^n + B_N^n) ...
          - v_mP * M_P/(K_mP + M_P) - k_dmp * M_P;
% ===============================================================
