...
%% analysis2_nullcline.m
optsODE = odeset('RelTol',1e-8,'AbsTol',1e-10);

% 积分到稳态，获得其余 14 个快变量的基值
[~,Y]  = ode15s(@(t,y) LG(t,y,p_array), [0 1000], y_array, optsODE);
y_ss   = Y(end,:)';

% 扫描区间（取稳态的 0–2 倍）
PCN_rng = linspace(0, 2*y_ss(9), 40);
BN_rng  = linspace(0, 2*y_ss(14), 40);
[PCN,BN] = meshgrid(PCN_rng, BN_rng);

dPCN = zeros(size(PCN));  dBN  = zeros(size(PCN));
...
figure; hold on
contour(PCN_rng,BN_rng,dPCN',[0 0],'r','LineWidth',1.4);
contour(PCN_rng,BN_rng,dBN' ,[0 0],'b','LineWidth',1.4);
plot(y_ss(9), y_ss(14),'ko','MarkerFaceColor','k');

% 画一条 200 h 真实轨迹（稍偏离平衡）
[~,Ytraj] = ode15s(@(t,y) LG(t,y,p_array), [0 200], y_ss.*1.05, optsODE);
plot(Ytraj(:,9), Ytraj(:,14),'k-');

xlabel('PC_N'); ylabel('B_N');
legend('dPC_N/dt=0','dB_N/dt=0','Equilibrium','Trajectory');
title('PC_N–B_N Nullclines and Trajectory');
grid on
